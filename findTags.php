		...
		// loop through the data
			while ($row = mysqli_fetch_assoc($result)) {
				...
				if($row['Privacy'] == 0) {
					...
					echo '<a href="imagedetail.php?image=' . $row['ImageID'] . '"><h4 align ="center">' . $row['Title'] . '</h4></a><p> by <a href="profile.php?UID=' . $row['UID'] . '">' . $row['Username'] . '</a></p>';
					echo '<a href="imagedetail.php?image=' . $row['ImageID'] . '"><img src="' . $row['Path'] . '" width="35%" alt="' . $row['Title'] . '" title="' . $row['Title'] . '"></a>';
					echo '<br/><br/>';
